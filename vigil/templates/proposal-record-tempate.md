...
* A proposal ordinarily declares one `repair_scope.primary_failure_mode`, or none where it arose directly from research or governance design. Additional links belong in `linked_records.contextual_relations` with `chain_inclusion: false` and never imply joint repair.
