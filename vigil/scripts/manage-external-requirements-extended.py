...
DATASET_PROVENANCE={'content_origin':'ai-authored','generation_mode':'semi-autonomous','human_role':'contract-approver','human_authorship':False,'human_review_status':'not-reviewed','human_verification_status':'not-verified','declaration':PROVENANCE_REF}

def generated_provenance(upstream):
    return {'content_origin':'deterministically-generated','generation_mode':'deterministic-generation','human_role':'contract-approver','human_authorship':False,'human_review_status':'not-reviewed','human_verification_status':'not-verified','declaration':PROVENANCE_REF,'upstream_provenance':upstream}

def mark_json_generated(content,upstream):
    document=json.loads(content); document['authorship_provenance']=generated_provenance(upstream); return jtxt(document)

def mark_markdown_generated(content,upstream):
    paths=', '.join(f'`{path}`' for path in upstream)
    return content.rstrip()+f"\n\n## Authorship provenance\n\nThis is a deterministically generated projection of {paths}. Its upstream VIGIL content is AI-authored and semi-autonomous under human contract approval; no human review or verification is implied. See `{PROVENANCE_REF}`.\n"
...
def scope(u):
    ...
    elif status=='complete': notes='Licensed IEEE primary text directly reviewed; VIGIL stores analytical abstractions and clause identifiers, not licensed standard text.'; detail=u.get('extraction_scope_notes','Governance-relevant normative or recommended-practice content was decomposed from the supplied primary text.'); un=[]; nxt='Monitor the source for amendment, corrigendum, supersession, or other material revision.'
    elif status=='partial': notes='Licensed IEEE primary text directly reviewed; VIGIL stores abstractions, not standard text.'; detail=u.get('extraction_scope_notes','A bounded direct-source extraction is represented; identified remaining normative content is not yet represented.'); un=u.get('known_unreviewed_sections',['Normative content outside the represented extraction scope']); nxt=u.get('next_action','Continue bounded clause-level extraction.')
...
def direct_req(c,src,defaults):
    if isinstance(c,list): c={'clause':c[0],'identity_key':c[1],'summary':c[2]}
    posture=c.get('posture',defaults['posture']); etype=c.get('expectation_type',defaults['expectation_type'])
    r=req_common(src,defaults['version'],c['clause'],c['identity_key'],c['summary'],posture,etype,c.get('actors',defaults['actors']),c.get('objects',defaults['objects']),c.get('stages',defaults['stages']),c.get('concepts',defaults['concepts']),'direct-licensed-primary','licensed-primary-text','Direct clause-level analysis of the supplied licensed IEEE primary text; the requirement is analytically paraphrased and the licensed source text is not reproduced.',c.get('review_limitations',defaults['review_limitations']))
    ...
    r['evidence_expectation']=c.get('evidence',[]); r['timing_or_frequency']=c.get('timing',[]); r['required_artefacts']=c.get('artefacts',[]); r['verification_method']=c.get('verification',[])
    r['applicability_conditions']=c.get('conditions',defaults.get('conditions',[])); r['exceptions_or_qualifications']=c.get('qualifications',defaults.get('qualifications',[]))
    r['source_defined_tags']=[{'scheme':f"{defaults['source_tag']}-clause",'values':[c['clause']]}]
    r['interpretation_provenance']['source_locator']=defaults.get('source_locator',src['official_locator'])
    r['interpretation_provenance']['source_fingerprint']=defaults['source_fingerprint']
    return r
...
def effective():
    ...
    for p in packs():
        ...
        for rs in p.get('direct_requirement_sets',[]):
            src=byext.get((rs['external_source_id'],rs['source_version']))
            if not src: errors.append(f"direct requirement set references absent source {(rs['external_source_id'],rs['source_version'])}")
            else: reqs += [direct_req(c,src,rs['defaults']) for c in rs.get('requirements',[])]
    ...
    external_upstream=['vigil/external_requirements/requirements.json','vigil/external_requirements/extensions/*.json']
    source_upstream=['vigil/external_sources/ledger.json','vigil/external_requirements/extensions/*.json']
    out[base.INDEX_PATH]=mark_json_generated(out[base.INDEX_PATH],external_upstream)
    out[base.COMPLETENESS_PATH]=mark_json_generated(out[base.COMPLETENESS_PATH],external_upstream)
    for path in (base.CATALOGUE_PATH,base.ACCESS_PATH,base.PRIORITY_PATH): out[path]=mark_markdown_generated(out[path],external_upstream)
    out[ELED]=jtxt({'schema_version':'1.0','updated_at':date,'generated_from':['external_sources/ledger.json','external_requirements/extensions/*.json'],'authorship_provenance':DATASET_PROVENANCE,'generation_provenance':generated_provenance(source_upstream),'entries':srcs}); out[EREQ]=jtxt({'schema_version':'1.1','updated_at':date,'generated_from':['external_requirements/requirements.json','external_requirements/extensions/*.json'],'authorship_provenance':DATASET_PROVENANCE,'generation_provenance':generated_provenance(external_upstream),'requirement_count':len(reqs),'requirements':sorted(reqs,key=lambda x:x['requirement_id'])})
    sl=['# Effective External Governance Sources','','Frozen Layer 0 plus reviewed extension sources. Inventory only; no Caelestis conformity is asserted.','',f'- Effective source versions: {len(srcs)}',f'- Reviewed through: {date}','','| VIGIL Source | External Source | Version | Issuer | Lifecycle | Canonical identifier |','| --- | --- | --- | --- | --- | --- |']+[f"| `{x['vigil_source_id']}` | {x.get('title') or ''} | `{x['source_version']}` | {x['issuer']} | `{x['source_lifecycle_state']}` | `{x['canonical_identifier']['value']}` |" for x in srcs]+['','Private-sector frameworks retain their actual publisher authority; inclusion does not elevate them to standards or regulatory authority.','']; out[EVIEW]=mark_markdown_generated('\n'.join(sl),source_upstream)
    xwalk=sorted(xwalk,key=lambda x:x['crosswalk_id']); out[XDATA]=jtxt({'schema_version':'1.0','updated_at':date,'authorship_provenance':DATASET_PROVENANCE,'generation_provenance':generated_provenance(external_upstream),'crosswalk_count':len(xwalk),'crosswalks':xwalk}); out[XIDX]=jtxt({'schema_version':'1.0','generated_at':date,'authorship_provenance':generated_provenance(['vigil/external_requirements/derivative-crosswalks.json']),'crosswalk_count':len(xwalk),'mapping_row_count':sum(len(x.get('mappings',[])) for x in xwalk),'crosswalks':[{'crosswalk_id':x['crosswalk_id'],'mapping_name':x['mapping_name'],'mapping_version':x['mapping_version'],'mapping_status':x['mapping_status'],'relationship_type':x['relationship_type'],'reference_document':x['reference_document'],'focal_document':x['focal_document'],'mapping_locator':x['mapping_locator'],'ingested_row_count':len(x.get('mappings',[])),'row_ingestion_status':x['completeness']['row_ingestion_status'],'may_assert_conformance':False} for x in xwalk]})
    ...
    out[XVIEW]=mark_markdown_generated('\n'.join(xl).rstrip()+'\n',['vigil/external_requirements/derivative-crosswalks.json']); return out,errors
