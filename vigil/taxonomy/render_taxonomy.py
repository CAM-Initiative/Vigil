...
def subtype_html(item: dict, *, heading: str) -> str:
    ...
    for subtype in item.get("subtypes", []):
        ...
        rows.append(
            ...
            f"<{heading} class=\"subtype-title\">{esc(subtype['name'])}</{heading}>"
    ...
    if not rows:
        return ""
    return '<section class="subtypes"><h3 class="subtypes-heading">Non-selectable subtypes and recognition patterns</h3>' + "".join(rows) + "</section>"
