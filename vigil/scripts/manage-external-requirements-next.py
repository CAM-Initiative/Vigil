...
def build_outputs(
    ...
    for row in limited:
        ...
        access_lines.append(f"| {row['title']} | `{row['source_version']}` | `{row['source_role']}` | `{row['source_access_status']}` | `{row['extraction_status']}` | {inaccessible.replace('|', '\\|')} | {action.replace('|', '\\|')} |")
    access_lines.append("")
    ...
    for row in sorted(blocked, key=lambda x: (priority_order[x["review_priority"]], x["title"], x["source_version"])):
        priority_lines.append(f"| `{row['review_priority']}` | {row['title']} | `{row['source_version']}` | {row['review_priority_rationale'].replace('|', '\\|')} | {row['source_access_status']} | {row['next_action'].replace('|', '\\|')} |")
