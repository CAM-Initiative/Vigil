...
WITHDRAWN_CLASS_OUTPUT_PATHS = [
    VIGIL_DIR / "VIGIL.Proposals.Index.json",
    VIGIL_DIR / "VIGIL.PatchNotes.Index.json",
]
NOTICE = (
...
def remove_withdrawn_class_outputs() -> None:
    """Keep retired PROP/PATCH indexes outside the maintained publication surface."""
    for path in WITHDRAWN_CLASS_OUTPUT_PATHS:
        path.unlink(missing_ok=True)
...
def build() -> None:
    ...
    remove_withdrawn_class_outputs()
    write_json(TAXONOMY_EXAMPLES_PATH, taxonomy_examples(all_records))
    print(f"Wrote {TAXONOMY_EXAMPLES_PATH}.")
