...
The generated report now assesses 947 records: 845 canonical records and 102 staged EU AI Act migration candidates. It records 420 metadata-complete records, 527 records still requiring review, 4,060 unresolved field decisions and 87 canonical records queued for substantive re-extraction, constituent enrichment or locator repair.
...
`reextraction-backlog.json` contains 87 unique canonical requirement IDs:
...
- 72 `compound-normative-propositions` findings;
- 84 `constituent-semantics-loss` findings;
...
| Command | Result |
|---|---|
| `python vigil/scripts/seed-reviewed-source-metadata.py --write` (twice) | PASS; 404 reviewed non-EU requirements, 74 then 0 new entries, 87 deterministic backlog entries; ledger and backlog hashes unchanged on the second write |
| `python vigil/scripts/validate-external-requirement-metadata.py` | PASS; 947 records, 420 metadata-complete, 4,060 unresolved fields, 87 backlog records |
...
| `python vigil/scripts/validate-external-requirement-fidelity.py` | PASS; 6 explicit fidelity entries and 16 expected effective-downgrade warnings for historically complete but not fidelity-assured sources |
