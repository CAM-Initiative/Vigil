...
if os.environ.get("GITHUB_EVENT_NAME") == "push" and os.environ.get("GITHUB_REF_NAME") == TARGET_BRANCH:
    ...
    completed = subprocess.run(
        [sys.executable, "vigil/scripts/validate-vigil-records.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    subprocess.run(
        ["git", "add", "vigil/scripts/test_validate_vigil_records.py", ".github/Indices/vigil-full-validation-output.txt"],
        cwd=ROOT,
        check=True,
    )
    subprocess.run(
        ["git", "commit", "-m", "Capture full VIGIL record validation output"],
        cwd=ROOT,
        check=True,
    )
    subprocess.run(
