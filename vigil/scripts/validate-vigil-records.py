...
def validate_repair_relationships(path: Path, record: dict[str, Any], errors: list[str]) -> None:
    """Keep authoritative repair edges separate from visible contextual relationships."""
    linked = record.get("linked_records")
    if not isinstance(linked, dict):
        return
    ...
    if not isinstance(contextual, list):
        errors.append(f"{path}: linked_records.contextual_relations must be an array when present")
    else:
        for index, relation in enumerate(contextual):
            label = f"linked_records.contextual_relations[{index}]"
            if not isinstance(relation, dict):
                errors.append(f"{path}: {label} must be an object")
                continue
            record_id = relation.get("record_id")
            if not isinstance(record_id, str) or not record_id.strip():
                errors.append(f"{path}: {label}.record_id must be a non-empty string")
            if not isinstance(relation.get("relationship"), str) or not relation["relationship"].strip():
                errors.append(f"{path}: {label}.relationship must be a non-empty string")
            if relation.get("chain_inclusion") is not False:
                errors.append(f"{path}: {label}.chain_inclusion must be false")
            if not isinstance(relation.get("rationale"), str) or not relation["rationale"].strip():
                errors.append(f"{path}: {label}.rationale must be a non-empty string")
            if isinstance(record_id, str) and record_id in authoritative_ids:
                errors.append(f"{path}: {label}.record_id must not also appear in an authoritative linked-record array")

    scope = record.get("repair_scope")
    if scope is None:
        return
    if record.get("record_type") not in {"proposal", "patch", "patch_note"}:
        errors.append(f"{path}: repair_scope is reserved for proposal and PATCH records")
        return
    if not isinstance(scope, dict):
        errors.append(f"{path}: repair_scope must be an object")
        return
    primary = scope.get("primary_failure_mode")
    additional = scope.get("additional_resolved_failure_modes")
    exception = scope.get("multi_failure_mode_exception")
    rationale = scope.get("exception_rationale")
    if primary is not None and (not isinstance(primary, str) or not primary.strip()):
        errors.append(f"{path}: repair_scope.primary_failure_mode must be a non-empty failure-mode ID or null")
    if not isinstance(additional, list) or any(not isinstance(value, str) or not value.strip() for value in additional):
        errors.append(f"{path}: repair_scope.additional_resolved_failure_modes must be an array of non-empty IDs")
        additional = []
    ...
    if exception is True:
        if not isinstance(rationale, str) or not rationale.strip():
            errors.append(f"{path}: repair_scope.exception_rationale is required for a multi-failure-mode exception")
    elif rationale not in {None, ""}:
        errors.append(f"{path}: repair_scope.exception_rationale must be null unless a multi-failure-mode exception is declared")
    declared = [value for value in [primary, *additional] if isinstance(value, str) and value]
    related_failures = linked.get("related_failure_modes", [])
    if isinstance(related_failures, list) and any(value not in related_failures for value in declared):
        errors.append(f"{path}: each declared repair_scope failure mode must appear in linked_records.related_failure_modes")
    if exception is False and additional:
        errors.append(f"{path}: additional resolved failure modes require multi_failure_mode_exception: true")
...
def validate_record(
    ...
    validate_repair_relationships(path, record, errors)
