...
if os.environ.get("GITHUB_EVENT_NAME") == "push" and os.environ.get("GITHUB_REF_NAME") == TARGET_BRANCH:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "vigil/scripts/validate-vigil-patch-trace.py",
            "--caelestis-root",
            "caelestis",
        ],
        cwd=ROOT,
    ...
    subprocess.run(
        ["git", "add", "vigil/scripts/test_validate_vigil_records.py", ".github/Indices/vigil-patch-trace-output.txt"],
        cwd=ROOT,
        check=True,
    )
    subprocess.run(
        ["git", "commit", "-m", "Capture final VIGIL PATCH trace output"],
        cwd=ROOT,
        check=True,
    )
    subprocess.run(
