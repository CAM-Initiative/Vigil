...
FACET_VALIDATOR = ROOT / "vigil" / "scripts" / "validate-failure-mode-facets.py"
RECORD_VALIDATOR = ROOT / "vigil" / "scripts" / "validate-vigil-records.py"

facet_spec = importlib.util.spec_from_file_location("validate_failure_mode_facets", FACET_VALIDATOR)
facet_validator = importlib.util.module_from_spec(facet_spec)
assert facet_spec.loader is not None
facet_spec.loader.exec_module(facet_validator)

record_spec = importlib.util.spec_from_file_location("validate_vigil_records_for_facets", RECORD_VALIDATOR)
record_validator = importlib.util.module_from_spec(record_spec)
assert record_spec.loader is not None
record_spec.loader.exec_module(record_validator)
...
class FailureModeFacetsTest(unittest.TestCase):
    ...
    def _write(self, record):
        ...
        path = Path(temp_dir.name) / f"{record['id']}.json"
        path.write_text(json.dumps(record), encoding="utf-8")
        return temp_dir, path

    def _validate_facets(self, record):
        temp_dir, path = self._write(record)
        try:
            return facet_validator.validate(path)
        finally:
            temp_dir.cleanup()
    ...
    def _validate_record(self, record):
        temp_dir, path = self._write(record)
        try:
            return record_validator.validate(path)
        finally:
            temp_dir.cleanup()
    ...
    def _valid_facets(self):
        return {
            ...
            "mechanism_or_cause": [
                "Completion assessment relied on internal narration rather than external state verification."
            ],
    ...
    def test_legacy_failure_without_facets_remains_valid(self):
        self.assertEqual(self._validate_facets(self._record()), 0)
    ...
    def test_missing_required_facet_is_rejected(self):
        record = self._record()
        facets = self._valid_facets()
        facets.pop("propagation")
        record["failure_classification"]["faceted_analysis"] = facets
        self.assertNotEqual(self._validate_facets(record), 0)
