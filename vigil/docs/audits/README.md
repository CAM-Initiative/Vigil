...
- `records/` — public-record evidence-integrity, consistency, lifecycle and registry audits.
- `schema/` — schema-authority, contract-consolidation and validator-alignment audits.
...
2. `vigil/scripts/` is reserved for repeatable current builders, validators, managers, routers, active auditors, active seeders, and explicitly retained maintenance tools;
...
5. a one-off migration, apply, reconciliation or seeding script must be reviewed for retirement after its result is canonical and independently validated; do not delete it while current tests, documentation, recovery procedures or repeatable maintenance still depend on it;
6. path moves must update workflows and documented commands in the same administrative change; and
7. housekeeping-only changes do not create VIGIL PROP, PATCH, FM, OBS, RESEARCH or other substantive governance records.

The TAXONOMY-04A/04B/04C sequence is intentionally retained as transition history. Its presence records how the taxonomy architecture evolved; it does not make those intermediate architectures current.
